...
namespace PizzaOrderingApplication.Controllers
{
    public class NonVegController : Controller
    {
        ...
        public NonVegController(PizzaContext context, ILogger<VegController> logger, IRepo<PizzaDetails> repo)
        {
            _context = context;
            _logger = logger;
            _repo = repo;
        ...
        /// <summary>
        /// Here in this page or controller the data will show the filtered data of Non-Vegetarian pizzas.
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        public IActionResult Index()
        {
            try
            {
                var res = _context.PizzaDetails.Where(i => i.PizzaType == "Non-veg").ToList();
                return View(res);
            }
            catch (Exception e)
            {
